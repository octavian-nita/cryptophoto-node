 ...
 * as second argument. If <code>process.env.CP_VISIBLE_IP</code> is set no further detection takes place and callback is
 * called in the next tick. Intended mostly for testing purposes.
 */
function visibleIp(callback) {
  if (process.env.CP_VISIBLE_IP) {
    return process.nextTick(function() { callback(null, process.env.CP_VISIBLE_IP); });
  }
